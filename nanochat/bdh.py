...
class Attention(torch.nn.Module):
    ...
    def forward(self, Q, K, V, freqs, state=None, t_offset=0):
...
class BDH(nn.Module):
    ...
    def __init__(self, config: BDHConfig):
        ...
        nh = config.n_head
        D = config.n_embd
        self.N = config.mlp_internal_dim_multiplier * D // nh

        self.lm_head = nn.Linear(D, config.vocab_size)

        self.decoder = nn.Linear(nh * self.N, D, bias = False)

        # Just stick with parameter, 
        self.encoder = nn.Parameter(torch.zeros((nh, D, self.N)).normal_(std=0.02))
        ...
        self.encoder_v = nn.Parameter(torch.zeros((nh, D, self.N)).normal_(std=0.02))
