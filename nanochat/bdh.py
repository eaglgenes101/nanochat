...
# With patches from adamskrodzki:feature/bdh-enhancements-clean
...
class BDH(nn.Module):
    def __init__(self, config: BDHConfig):
        ...
        D = config.n_embd
        N = config.mlp_internal_dim_multiplier * D // nh
        self.decoder = nn.Parameter(torch.zeros((nh * N, D)).normal_(std=0.02)) # Matrix
        self.encoder = nn.Parameter(torch.zeros((nh, D, N)).normal_(std=0.02)) # One matrix per head
        ...
        self.ln = nn.LayerNorm(D, elementwise_affine=False, bias=False) # Layer norm
        self.embed = nn.Embedding(config.vocab_size, D) # Embedding
        self.drop = nn.Dropout(config.dropout)
        self.encoder_v = nn.Parameter(torch.zeros((nh, D, N)).normal_(std=0.02)) # One matrix per head
